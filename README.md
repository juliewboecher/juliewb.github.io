...
Tjek først din Node-version:

```bash
node --version
```

Node-versionen skal være 22.22 eller nyere.

Installer derefter projektet:
...
Installer Node.js 22.22 eller nyere, åbn terminalen igen, og prøv derefter `npm install`.

### Deployment fejler med `npm ci` eller `EUSAGE`

Fejlen betyder normalt, at `package-lock.json` ikke passer til projektets dependencies. Du skal ikke installere de pakker, der står som `Missing`, manuelt.

Hvis dit repository er oprettet fra en ældre version af templaten, skal du også åbne `.github/workflows/deploy.yml` og ændre:
...
Tjek først med `node --version`, at du har Node.js 22.22 eller nyere. Opdater ellers Node.js og åbn terminalen igen. Luk også en eventuel dev-server med `Ctrl + C`.
...
På Windows PowerShell skal du køre:
...
Commit og push derefter den nye lockfil:
...
GitHub Actions starter automatisk igen efter dit push.
